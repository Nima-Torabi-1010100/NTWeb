(function () {
    "use strict"; const headerToggleBtn = document.getElementById('header-toggle'); const header = document.getElementById('header'); const toggleWrapper = document.getElementById('header-toggle-wrapper'); headerToggleBtn.addEventListener('click', () => { const isActive = header.classList.toggle('header-show'); headerToggleBtn.classList.toggle('active', isActive); toggleWrapper.classList.toggle('active', isActive) }); window.addEventListener('load', function () { window.scrollTo(0, 0) }); document.querySelectorAll('#navmenu a').forEach(link => { link.addEventListener('click', () => { header.classList.remove('header-show'); headerToggleBtn.classList.remove('active'); toggleWrapper.classList.remove('active') }) }); document.querySelectorAll('.navmenu .toggle-dropdown').forEach(navmenu => { navmenu.addEventListener('click', function (e) { e.preventDefault(); this.parentNode.classList.toggle('active'); this.parentNode.nextElementSibling.classList.toggle('dropdown-active'); e.stopImmediatePropagation() }) }); const preloader = document.querySelector('#preloader'); if (preloader) { window.addEventListener('load', () => { preloader.classList.add('hidden'); setTimeout(() => { preloader.remove() }, 300) }) }
    ...
    window.addEventListener('load', navmenuScrollspy); document.addEventListener('scroll', navmenuScrollspy); const popup = document.getElementById("popupMessage"); const form = document.getElementById("form"); if (popup && form) { const formRect = form.getBoundingClientRect(); popup.style.left = `${formRect.left + formRect.width / 2}px`; popup.style.maxWidth = `${formRect.width}px` }
    const footerImage = document.querySelector(".footer-image"); if (footerImage) {
        let ticking = !1; function updateFooterImage() {
            const scrollY = window.scrollY; const docHeight = document.documentElement.scrollHeight; const winHeight = window.innerHeight; const scrollBottom = docHeight - (scrollY + winHeight); if (scrollBottom < 120) { footerImage.classList.add("show") } else { footerImage.classList.remove("show") }
            ticking = !1
        }
        window.addEventListener("scroll", () => { if (!ticking) { window.requestAnimationFrame(updateFooterImage); ticking = !0 } }); updateFooterImage()
    }
    const faqItems = document.querySelectorAll('.faq-item'); if (faqItems.length > 0) { faqItems.forEach(item => { const question = item.querySelector('.faq-question'); question.addEventListener('click', () => { item.classList.toggle('active') }) }) }
})()